...
function exportToWord() {
    if (!currentWord || resultsList.children.length === 0) return;

    let rhymes = [];
    Array.from(resultsList.children).forEach(chip => {
        if (chip.classList.contains('rhyme-chip')) {
            rhymes.push(chip.textContent);
        }
    });

    if (rhymes.length === 0) return;

    let title = currentWord;
    let subtitle = currentTopic && currentTopic !== 'Listening...' ? `Topic: ${currentTopic}` : '';
